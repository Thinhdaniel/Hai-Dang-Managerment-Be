...
export const serializePlant = (input: any) => {
    ...
    return {
        ...
        productionAccess: {
            ...
            stage: productionAccess?.stage || (productionAccess?.enabled === true ? 'live' : 'disabled'),
            previousStage: productionAccess?.previousStage,
            revision: Number(productionAccess?.revision || 0),
            wave: productionAccess?.wave,
            plannedGoLiveDate: productionAccess?.plannedGoLiveDate,
            ownerName: productionAccess?.ownerName,
            acceptedPilotRunId: toId(productionAccess?.acceptedPilotRunId),
            acceptedPilotCode: productionAccess?.acceptedPilotCode,
            lastTransitionAt: toIso(productionAccess?.lastTransitionAt),
            lastTransitionBy: toId(productionAccess?.lastTransitionBy),
            lastTransitionByName: productionAccess?.lastTransitionByName,
            lastTransitionReason: productionAccess?.lastTransitionReason,
